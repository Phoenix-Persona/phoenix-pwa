...
LNURL-pay endpoint for you.** No Zuka-owned domain, no static JSON
...
on the same domain will replace the previous one."* Each Zuka
persona has its own wallet (its own Spark identity), so this is per
*Spark identity*, not per Zuka operator — multiple personas under
...
Zuka uses PPQ's **credits system** (see `./ppq.md`): the
persona's Spark wallet keeps PPQ's `credit_id` topped up via NIP-47
NWC. Zuka wires this once at persona creation by handing PPQ an
...
| `src/lib/wallet/client.ts`               | Zuka-flavoured wallet ops over the SDK         |
...
## `WalletInfo` shape (Zuka-flavoured)
